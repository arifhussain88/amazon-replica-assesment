      ...
      <div className="bg-primary">
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-3 py-2 lg:gap-4 lg:px-6">
          ...
          <p className="hidden text-xs leading-tight sm:block">
            <span className="block text-primary-foreground/75">Deliver to</span>
          ...
          <Link href={user ? "/orders" : "/account?next=/orders"} className="hidden text-sm leading-tight sm:block">
            <span className="block text-xs text-primary-foreground/75">{user ? `Hello, ${firstName}` : "Hello, sign in"}</span>
      ...
      <div className="bg-primary-hover">
        <div className="mx-auto flex max-w-7xl items-center gap-2 overflow-x-auto px-3 py-2 lg:px-6">
          ...
          {categories.map((category) => (
            <Link key={category.slug} href={`/search?category=${category.slug}`} className="shrink-0 px-2 text-sm hover:outline hover:outline-primary-foreground">
              {category.name}
            </Link>
          ))}
        </div>
